...
shinyServer(function(input, output, session) {
  ...
  outputFileNames <- reactive({
    ...
    archive <- paste("SLEEP_TutoralA_", gsub(".csv", "", sleepIntervals()$name), "_Archive_", initials(), "_", tmsp, ".csv", sep="")
  ...
  macro0 <- eventReactive(input$runA, {
    ...
    # Read in data
    sleepIntervalsOrig <- read.csv(sleepIntervals()$datapath, sep=",", header=FALSE, skip=1)
    sleepIntervalsData <- sleepIntervalsOrig
    sleepIntervalsData$StartTimeL <- parse_date_time(as.character(sleepIntervalsData[,11]), "HMS")
    sleepIntervalsData$StartDateL <- parse_date_time(as.character(sleepIntervalsData[,9]), "mdy")
    ...
    # DataStartDate
    datastartdate <- as.character(sleepIntervalsData[,"DataStartDate"])
    datastartdate <- parse_date_time(datastartdate, "mdy")
    ...
    # DataStartTime
    sleepIntervalsData[,"DataStartTime_A"] <- sleepIntervalsData[,"DataStartTime"]
    ...
    # StartDate
    startdate <- as.character(sleepIntervalsData[,"StartDate"])
    startdate <- parse_date_time(startdate, "mdy")
    ...
    # StartTime
    sleepIntervalsData[,"StartTime_A"] <- sleepIntervalsData[,"StartTime"]
    ...
    # EndDate
    enddate <- as.character(sleepIntervalsData[,"EndDate"])
    enddate <- parse_date_time(enddate, "mdy")
    ...
    # EndTime
    sleepIntervalsData[,"EndTime_A"] <- sleepIntervalsData[,"EndTime"]
    ...
    # For Mac
    archivePath <-  paste("~/Desktop/psychophys/archive", outputFileNames()["outputDirectory"], sep="/")
